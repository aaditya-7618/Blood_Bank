...
public class RecipientAppointmentDao {
	...
	public List<RecipientAppointment> getAllRecipientAppointment() {
		...
		List<RecipientAppointment> allrecipientlist = jdbcTemplate.query(sql,
				new BeanPropertyRowMapper<RecipientAppointment>(RecipientAppointment.class));
	...
	public void update(String appointmentID, String userName, String bankID, String bloodGrp, int bloodUnit,
			LocalDate date, LocalTime time) {
		jdbcTemplate.update(
				"update recipientappointments set appointmentID=?, userName =?, bankID=? ,bloodGrp=?,bloodUnit=?,date=? ,time=? where appointmentID=? ",
				appointmentID, userName, bankID, bloodGrp, bloodUnit, date, time, appointmentID);
	}
}
